...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: As per the function's # Safety documentation, `address` is guaranteed to be
    // the address of a valid, mutable `u32` value. Converting it to *mut u32 is safe,
    // and dereferencing the pointer to modify the value complies with the mutable requirement.
    unsafe {
